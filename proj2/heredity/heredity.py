    ...
    for person, attr in people.items():
        ...
        # if he got parents, we need to infer his prob by compute parents prob
            ...
            if gene_num == 0:
                ...
                p *= (1 - parents[mom]) * parents[dad] + parents[mom] * (
                    1 - parents[dad]
                )
            else:
                p *= parents[mom] * parents[dad]

        p *= PROBS["trait"][gene_num][is_trait]
    return p
...
def normalize(probabilities):
    ...
    """
    for person, attr in probabilities.items():
        total_gene = sum(attr["gene"].values())
